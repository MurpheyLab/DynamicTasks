...
//---------------------------------------------------------------------
#ifndef _HAPTIC_UTILITY2_H_
#define _HAPTIC_UTILITY2_H_
...
void InitializeDevice(long dev) {
	...
	if (haSendString(dev, "set inertia 6.0", outputString)) {
  	printf("--- ERROR: set inertia: %s\n", outputString);
  }

  char isCalibratedStr[10] = "";
  if (haSendString(dev, "get position_calibrated", outputString)) {
  ...
  if (!strcmp(isCalibratedStr, "false")) {
   	if (haSendString(dev, "set state init", outputString)) {
    	printf("--- ERROR: set state init: %s\n", outputString);
   	} else {
    	printf( "Initializing the HapticMASTER. Please wait...\n" );
    }

	  if (haSendString(dev, "get state", outputString)) {
	...
	if (haSendString(dev, "set state force", outputString)) {
...
int  haSendCommand(long inDev,
	...
	char* outCommand) {
	return (haSendString(inDev, inCommand, outCommand));
}
...
int  haSendCommand(long inDev,
	...
	return (haSendString(inDev, tempString, outCommand));
}
