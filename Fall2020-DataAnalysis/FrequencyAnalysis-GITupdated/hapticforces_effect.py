...
save_values = 0 # 0-do not save 1-save
...
DIR = "Z:\hapticfeedback_exp" #set directory where data is mounted Ola- "/media/ola/Elements/R01prelim" Milli -"Z:"
number_of_subjects = 1 # in each group, controls and stroke
DT = 0.01
Fs = 1/DT
...
for group in range(len(group_name)):
    ...
    if True:
        ...
        if (group_name[group] == 'Stroke'):
            ...
            plt.title('Subject Without Haptic Feedback: Fmag')
        else:
            subject_num = 17
            # subname = "S" + str(subject_num)
            plt.figure(subject_num)
            plt.title('Subject With Haptic Feedback: Fmag')
        ...
        for j in range(0,3): #iterate through support levels
            ...
            for k in range(1,46): #iterate through trials
                for i in range(1,6): #iterate through tasks
                    try:
                        ...
                        trialfile = DIR+subfile+"_Task"+str(i)+"_SL"+str(j)+"_Trial"+ str(k)+".csv"
                        ...
                        if save_values==1 or make_plots==1:
                            ...
                            for w_i in range(0,len(w)):
                                ...
                                if (w[w_i]>1):#freq_pendulum+1) and (w[w_i]<10):
                            ...
                            # print(np.sum(np.square(freq_list))*dw)
                            data_boxplot[ratio_index,supportlevel_index] += np.sum(np.square(freq_list))*dw
                            # data_boxplot[ratio_index,supportlevel_index] += np.log(np.sum(freq_list)*dw)
                            # print(data_boxplot)
                            ...
                            Fx_high_content = data_boxplot[ratio_index,supportlevel_index]
            ...
            for w_i in range(0,len(w)):
                if (w[w_i] < 2*freq_pendulum+1) and (w[w_i] > 2*freq_pendulum-1):
                    freq_list2.append(A_support[w_i])
            areaunder = np.sum(np.square(freq_list2))*dw
            print(subject_num, areaunder)
...
if make_plots==1:
    ...
    num_per_support = 3*5
    # data_boxplot = data_boxplot_low
    data_boxplot /= num_per_support
    data_boxplot = data_boxplot.transpose().tolist()
    print(data_boxplot)

    # for controls vs. stroke
    stroke_all = np.array([data_boxplot[0],data_boxplot[2],data_boxplot[4]])
    controls_all = np.array([data_boxplot[1],data_boxplot[3],data_boxplot[5]])
    ...
    print('stroke vs. controls',d)

    # for repeated measures stroke
    mean1 = np.mean(data_boxplot[0])
    mean2 = np.mean(data_boxplot[4])
    std1 = np.std(data_boxplot[0])
    std2 = np.std(data_boxplot[4])
    std_pooled = np.sqrt((std1**2+std2**2)/2)
    print(mean1,mean2,std1,std2,std_pooled)
    d = abs((mean1-mean2)/std_pooled)
    print('repeated measures stroke',d)

    # for repeated measures controls
    mean1 = np.mean(data_boxplot[1])
    mean2 = np.mean(data_boxplot[5])
    std1 = np.std(data_boxplot[1])
    std2 = np.std(data_boxplot[5])
    std_pooled = np.sqrt((std1**2+std2**2)/2)
    # print(mean1,mean2,std1,std2)
    d = abs((mean1-mean2)/std_pooled)
    print('repeated measures controls',d)

    # for severe vs. moderate
    severe = np.array([data_boxplot[0][0],data_boxplot[0][1],data_boxplot[0][5]])
    print(severe)
    moderate = np.array([data_boxplot[0][3],data_boxplot[0][4]])
    mean1 = np.mean(severe.flatten())
    mean2 = np.mean(moderate.flatten())
    std1 = np.std(severe.flatten())
    std2 = np.std(moderate.flatten())
    std_pooled = np.sqrt((std1**2+std2**2)/2)
    print(mean1,mean2,std1,std2,std_pooled)
    d = abs((mean1-mean2)/std_pooled)
    print('severe vs. moderate',d)

    power_data_x = np.zeros(6)
    power_data_y = np.array(data_boxplot[1])
    power_data_x = np.append(power_data_x,np.zeros(6)+1)
    power_data_y = np.append(power_data_y,data_boxplot[3])
    power_data_x = np.append(power_data_x,np.zeros(6)+2)
    power_data_y = np.append(power_data_y,data_boxplot[5])
    # [np.mean(data_boxplot[0]),np.mean(data_boxplot[2]),np.mean(data_boxplot[4])]
    print(power_data_x)
    print(power_data_y)
    slope, intercept, r_value, p_value, std_err = sp.stats.linregress(power_data_x,power_data_y)
    print(slope, intercept, r_value, p_value, std_err)
    # compute_R2(power_data)
    ...
    labels = ['Stroke','Controls','Stroke','Controls','Stroke','Controls']
    box_colors = ['#601A4A', '#601A4A','#EE442F','#EE442F', '#63ACBE', '#63ACBE']
    box_alpha = [None,0.3,None,0.3,None,0.3]
    make_boxplot(fig,ax1,data_boxplot,labels,box_colors,box_alpha)
    fig.subplots_adjust(bottom=0.2)
    combos = [[0,2],[2,4],[0,4],[1,3],[3,5],[1,5],[0,1],[2,3],[4,5]]
    sig_matrix = np.zeros((len(combos),3))
    for i in range(len(combos)):
        # [tstat,pval] = stats.ttest_ind(data_boxplot[combos[i][0]],data_boxplot[combos[i][1]], equal_var = True)
        [tstat,pval] = stats.ttest_rel(data_boxplot[combos[i][0]],data_boxplot[combos[i][1]])
        pval /= 2
        sig_matrix[i,:] = [combos[i][0],combos[i][1],pval]
    print(sig_matrix)
    # add_stats(ax1,data_boxplot,sig_matrix)
    ...
    ax1.set_title('High-Frequency Content For Each Loading Level', fontweight='bold',fontname="Arial", fontsize=11, color='black')
    # ax1.set_xlabel('Group - Loading Level (% Max)',fontname="Arial", fontsize=11)
    ...
    ax1.text(0.02, text_height,'Loading:',horizontalalignment='right',transform=ax1.transAxes, fontname="Arial", fontsize=10, fontweight='bold')
    ax1.text(0.02, -.09,'Group:',horizontalalignment='right',transform=ax1.transAxes, fontname="Arial", fontsize=10, fontweight='bold')
