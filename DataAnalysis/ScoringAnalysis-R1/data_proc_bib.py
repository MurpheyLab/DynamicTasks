...
list_of_participants = ["S02","S03","S04","S05","S06","S07"]
list_of_participants_num = ["2","3","4","5","6","7"]
...
def calc_metrics(trialfile):
    ...
    meanenergy = np.mean(data[:,16])
    #meanenergy = calc_ball_energy(data[:,4:7], len(data[:,0]))
    #print([score,tinbowl,meanenergy])
    return [score,tinbowl,meanenergy] # meanvel,maxvel,meanvellifted,meanjerk,maxjerk,intenergy,windowenergy]

def calc_ball_energy(pos, trial_len):
    vel = np.gradient(pos,axis=0)
    ballenergy_vec = np.zeros(trial_len, trial_len)
        ...
        for k in range(1,49): #trials
            for i in range(0,2): #forces
                try:
                    ...
                    row = calc_metrics(DIR+sub+subfile+"_Trial"+str(k)+"_Freq"+str(j)+"_SL0_F"+str(i)+".csv")
                    #print(row)
...
columns_all = ["Side","Dominance","Subject","TrialNum","Forces","Freq","Score","TIB","MeanEnergy"]
